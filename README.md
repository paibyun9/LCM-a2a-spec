...
Tier-1
Pre only
Intent evaluation (no proof exposure)

Tier-2
Post only
Ledger verification / auditing

Partner
Pre + Post
Full decision → proof pipeline
...
Health & Liveness   System availability and version lock
Pre — Intent Only   Deterministic decision intent   No evidence binding
Evidence Binding   Hash, signature, reference only
Post — Ledger Commit   Immutable proof generation
External Verification   Third-party reproducibility
